...
1. Starts a local Camunda7 instance
...
1. **`camunda7`** — starts the engine with an in-memory H2 database and waits until its REST API is up
...
## Connecting to Your Own Camunda7 Instance

To point the worker at an existing Camunda 7 engine instead of the local one, pass the
environment variables directly:

```shell
engine_url=https://<your-engine url>/engine-rest/ \
